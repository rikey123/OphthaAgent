...
from agents.fundus_tools_agent.measuremeng.quantitative_measurement import FundusQuantitativeMeasurement
...
# 定义可用眼底工具列表
tools = {
    "assess_image_quality": FundusImageQuality(),
    "preprocess_image": FundusImagePreprocessing(),
    "segment_anatomical_structures": FundusAnatomicalSegmentation(),
    "detect_lesions": FundusLesionDetection(),
    "measure_quantitative_metrics": FundusQuantitativeMeasurement(),
}
...
"segment_anatomical_structures": Segments key anatomical structures in a fundus image.
  tool_args:
    image_path (str): The path to the image file.
    structures (list[str]): A list of structures to segment. Valid structures: ['optic_disc', 'optic_cup'].

"detect_lesions": Detects various types of lesions in a fundus image.
  tool_args:
    image_path (str): The path to the image file.

"measure_quantitative_metrics": Measures quantitative metrics from segmentation results.
  tool_args:
    segmentation_results (dict): The output from 'segment_anatomical_structures'.
"""""
...
Now, create the plan as a JSON object that adheres to the `Plan` schema. For each step, you must provide a `tool_name`, the `tool_args`, and a `reason` explaining why this step is necessary.
"""
...
def execute_tool(tool_call: dict) -> Dict:
    ...
    try:
        ...
        if tool_name == "assess_image_quality":
            result = tool.assess_image_quality(**tool_args)
        elif tool_name == "preprocess_image":
            result = tool.preprocess_image(**tool_args)
        elif tool_name == "segment_anatomical_structures":
            result = tool.segment_anatomical_structures(**tool_args)
        elif tool_name == "detect_lesions":
            result = tool.detect_lesions(**tool_args)
        elif tool_name == "measure_quantitative_metrics":
            result = tool.measure_quantitative_metrics(**tool_args)
        else:
...
if __name__ == "__main__":
    ...
    image_path = "./agents/fundus_tools_agent/tools/test1.jpg"  # Replace with a real image path
    ...
    final_response = final_state.get("response", "No final response generated.")
