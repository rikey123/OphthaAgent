    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            if os.path.exists(img_path):
                ...
                # Try finding file with different case or extension if needed
                # But for now assume exact match
                pass
    return data

def map_label(label):
    # User defined mapping for DEEPDR
    # Dataset 0, 1 -> Model 0
    # Dataset 2, 3 -> Model 1
    # Dataset 4 -> Model 2
...
class MessidorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Return a dummy tensor and -1 label to indicate error
...
def main():
    ...
    dataloader = DataLoader(messidor_dataset, batch_size=16, shuffle=False, num_workers=0)
    ...
    try:
        with torch.no_grad():
            for i, (images, labels, paths) in enumerate(dataloader):
                if i == 0:
                    # Diagnostic: Check input tensor stats
                    print(f"\n--- Diagnostic: Batch {i} ---")
                ...
                for label, pred, path in zip(labels, preds, paths):
                    label = label.item()
                    if label == -1: # Skip error images
                        continue
                    
                    mapped_label = map_label(label)
                    # print(f"Image: {path}, True: {mapped_label}, Pred: {pred}")
