...
from predict_single_image import load_model, preprocess_image
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            if os.path.exists(img_path):
                ...
                # Try appending .png or .jpg if not found
                if os.path.exists(img_path + '.png'):
                    data.append((img_path + '.png', label))
                elif os.path.exists(img_path + '.jpg'):
                    data.append((img_path + '.jpg', label))
...
def main():
    ...
    parser.add_argument('--csv_path', type=str, default='<ANON_ABS_PATH>')
    parser.add_argument('--images_dir', type=str, default='<ANON_ABS_PATH>')
    parser.add_argument('--model_path', type=str, default='model/MKCNet_DeepDRiD_5Class_ResNet50/best_model.pth')
    parser.add_argument('--meta_model_path', type=str, default='model/MKCNet_DeepDRiD_5Class_ResNet50/best_model_LG.pth')
    ...
    with torch.no_grad():
        for i, (images, labels, paths) in enumerate(dataloader):
            ...
            # Forward pass
            # FirstOrder_MKCNet returns output, output_M, output_IQ
            # We only care about output (Task prediction)
