...
if os.path.exists('<ANON_ABS_PATH>'):
    shutil.rmtree('<ANON_ABS_PATH>') 
if os.path.exists('<ANON_ABS_PATH>'):
    shutil.rmtree('<ANON_ABS_PATH>') 
if os.path.exists('<ANON_ABS_PATH>'):
    shutil.rmtree('<ANON_ABS_PATH>')
...
Artery_PATH = '<ANON_ABS_PATH>'
Vein_PATH = '<ANON_ABS_PATH>'


for filename in sorted(glob.glob(os.path.join(CONFIG.image_directory, '*.png'))):
    segmentedImage = retina.Retina(None, filename, store_path='<ANON_ABS_PATH>')
    ...
    t1, t2, t3, t4, td, tfi, tft, vessel_density, average_caliber,vessel_count,tcurve, bifurcation_t, vessel_count_1, vessel_count_list, w1_list = tortuosity_measures.evaluate_window(window, CONFIG.pixels_per_window, CONFIG.sampling_size, CONFIG.r_2_threshold,store_path='<ANON_ABS_PATH>')
    #print(window.tags)
    t2_list.append(t2)
    t4_list.append(t4)
    t5_list.append(td)
