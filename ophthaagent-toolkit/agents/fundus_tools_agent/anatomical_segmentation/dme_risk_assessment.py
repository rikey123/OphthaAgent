...
def run_python_script(script_path: str, args: List[str], env_name: Optional[str] = None, cwd: Optional[str] = None) -> Dict:
    ...
    # 鲁棒的 JSON 提取逻辑：从后往前尝试解析最长的 JSON 块
    # 因为很多脚本会在输出主体 JSON 之前打印一些辅助信息或调试字典
    info = None
        ...
        for end in reversed(end_indices):
            if end > start:
                ...
                try:
                    ...
                    # 改进：放宽约束，不仅检查 status，也检查常用的 metadata 或 images (用于 AutoMorphalyzer)
                    if isinstance(temp_info, dict) and ("status" in temp_info or "metadata" in temp_info or "images" in temp_info):
                        info = temp_info
                        break
    ...
    if info is None:
        raise RuntimeError(f"未找到有效的 JSON 输出。原始输出内容:\n{stdout}")
...
def extract_hard_exudate_coordinates(hard_exudate_mask_path: str) -> List[np.ndarray]:
    """
    从硬性渗出物 mask 提取所有渗出物的轮廓点。
    由于 DR-Detection 系统输出的是拼接图（原图|间隔|Mask），我们需要提取右侧的 Mask。

    Args:
        hard_exudate_mask_path: 硬性渗出物分割结果图像路径

    Returns:
        渗出物在 512x512 空间中的点坐标列表 [ndarray(N, 1, 2), ...]
    """
    # 读取图像
    full_img = cv2.imread(hard_exudate_mask_path)
    if full_img is None:
        raise ValueError(f"无法读取硬性渗出物结果: {hard_exudate_mask_path}")

    # 提取右侧的 Mask 部分 (DR 系统固定使用 512x512)
    # 结果图通常是 512 | 10 | 512 -> 1034px 宽
    h, w = full_img.shape[:2]
    if w >= 1024:
        mask_part = full_img[:, -512:]
    else:
        mask_part = full_img

    # 转为单通道灰度图
    if len(mask_part.shape) == 3:
        mask = cv2.cvtColor(mask_part, cv2.COLOR_BGR2GRAY)
    else:
        mask = mask_part
    ...
    for contour in contours:
        # 调低面积过滤阈值，以捕获微小的早期硬性渗出 (Lower threshold to capture tiny early exudates)
...
def run_automorphalyzer(abs_image_path: str, output_dir: str) -> Dict:
    ...
    # 获取设备参数
    #device = os.environ.get("DME_DEVICE", "cuda:0")
    # 添加项目根目录到 sys.path
    ...
    if automorph_result.get("images") and len(automorph_result["images"]) > 0:
        ...
        disc_height_912 = optic_disc_data.get("disc_height_px", -1)
        disc_width_912 = optic_disc_data.get("disc_width_px", -1)

        if disc_height_912 > 0 and disc_width_912 > 0:
            # AutoMorphalyzer 输出的是 912x912 尺寸下的直径
            # 需要缩放到原始图像尺寸
            AUTOMORPH_SIZE = 912

            # 计算缩放比例
            # 改进：AutoMorphalyzer 通常将圆形眼底区域裁剪并缩放到 912x912
            # 缩放比例应基于原始图像的圆形区域直径（通常为 min(W, H)）
            circular_fundus_diameter = min(original_width, original_height)
            scale_factor = circular_fundus_diameter / AUTOMORPH_SIZE

            # 缩放到原始尺寸
            disc_height = disc_height_912 * scale_factor
            disc_width = disc_width_912 * scale_factor
            disc_diameter = (disc_height + disc_width) / 2

            print(f"  AutoMorphalyzer 输出 (912x912): 高度={disc_height_912:.1f}px, 宽度={disc_width_912:.1f}px")
            print(f"  缩放比例 (基于 min_dim={circular_fundus_diameter}): {scale_factor:.3f}")
...
def run_dr_detection(abs_image_path: str, output_dir: str) -> Dict:
    ...
    # 修复获取 analysis_ 目录的逻辑，应寻找最新创建的文件夹
    analysis_dirs = [d for d in os.listdir(dr_output_dir) if d.startswith("analysis_")]
    if not analysis_dirs:
        raise RuntimeError("DR 分析未生成结果目录")
    
    # 按照修改时间排序，取最新的一个 (Sort by modification time to get the latest one)
    analysis_dirs.sort(key=lambda x: os.path.getmtime(os.path.join(dr_output_dir, x)), reverse=True)
    latest_dir = analysis_dirs[0]
    ...
    # 改进：将 Mask 移动到外部并清理临时目录，节省磁盘空间 (Cleanup temporary analysis dirs)
    final_mask_path = os.path.join(dr_output_dir, "hard_exudates_mask_final.png")
    import shutil
    shutil.copy2(hard_exudate_mask_path, final_mask_path)
    # 删除临时生成的分析文件夹及其内容
    for d in analysis_dirs:
        shutil.rmtree(os.path.join(dr_output_dir, d))

    print("[任务 3/3] 完成: 硬性渗出物检测")
    return {"hard_exudate_mask_path": final_mask_path}


def calculate_min_distance_to_fovea(
    fovea_coord: Tuple[float, float],
    exudate_contours: List[np.ndarray],
    x_scale: float,
    y_scale: float
) -> Optional[float]:
    """
    计算所有硬性渗出物轮廓到黄斑中心凹的最小距离

    Args:
        fovea_coord: 黄斑中心凹坐标 (x, y)
        exudate_contours: 渗出物在 512x512 空间下的轮廓列表
        x_scale: x 方向缩放比例 (original_w / 512)
        y_scale: y 方向缩放比例 (original_h / 512)

    Returns:
        最小距离（像素），如果没有渗出物则返回 None
    """
    if not exudate_contours:
        return None

    min_dist = float('inf')
    f_x, f_y = fovea_coord

    for contour in exudate_contours:
        # 将轮廓点缩放到原始尺寸
        # contour 形状为 (N, 1, 2)
        ...
        # 计算该轮廓上所有点到 fovea 的距离
        dists = np.sqrt(np.sum((pts - np.array([f_x, f_y]))**2, axis=1))
...
def main(image_path: str, output_dir: str = None) -> Dict:
    ...
    try:
        ...
        exudate_contours = extract_hard_exudate_coordinates(hard_exudate_mask_path)
        
        # 记录 512x512 空间下的坐标用于内部处理，但在 main 中我们将直接计算距离
        x_scale = original_width / 512.0
        y_scale = original_height / 512.0
            
        print(f"✓ 检测到 {len(exudate_contours)} 个硬性渗出物轮廓\n")

        # 计算 DME 风险
        print("=" * 80)
        print("计算 DME 风险...")
        print("=" * 80)
        min_distance = calculate_min_distance_to_fovea(fovea_coord, exudate_contours, x_scale, y_scale)
        ...
        for cnt in exudate_contours:
            M = cv2.moments(cnt)
            if M["m00"] != 0:
                cx = (M["m10"] / M["m00"]) * x_scale
                cy = (M["m01"] / M["m00"]) * y_scale
                exudate_centers.append((cx, cy))
