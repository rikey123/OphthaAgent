...
3. 使用 dr_single_image_analyzer (Attention-UNet) 检测硬性渗出物
...
更新历史：
- 2025-01-21: 替换 DR-Detection-and-Clasification-System 为 dr_single_image_analyzer
  使用更先进的 Attention-UNet 模型提升分割精度
"""
...
# ============================================================================
# 全局设备配置 - 统一控制整个系统运行在哪个设备上
# ============================================================================

# 可选值: 'auto' (自动检测), 'cpu', 'cuda', 'cuda:0', 'cuda:1', 等
# 设置此变量后，所有子任务将使用相同的设备
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..'))
from device_config import get_device
device = get_device()
print(f"使用设备: {device}")
GLOBAL_DEVICE = str(device)  # 🔧 修改这里来切换设备

# 也可以通过环境变量 DME_DEVICE 控制（优先级高于上面的设置）
# 例如: export DME_DEVICE=cuda:0
...
def get_global_device():
    ...
    优先级: 环境变量 > GLOBAL_DEVICE 常量

    Returns:
        str: 设备字符串 ('auto', 'cpu', 'cuda:0', 等)
    """
    # 先检查环境变量
    env_device = os.environ.get('DME_DEVICE')
    if env_device:
        return env_device
...
def run_fovea_localization(abs_image_path: str, output_dir: str) -> Dict:
    ...
    fovea_result = run_python_script(
        fovea_script,
        [abs_image_path, abs_fovea_output,"--device", str(device)],
...
def run_automorphalyzer(abs_image_path: str, output_dir: str) -> Dict:
    ...
    automorph_result = run_python_script(
        automorph_script,
        ["--input", abs_image_path, "--output", automorph_output_dir, "--device", device],  # 🔧 使用全局设备配置
    ...
    if automorph_result.get("images") and len(automorph_result["images"]) > 0:
        ...
        if disc_height_912 > 0 and disc_width_912 > 0:
            ...
            # 计算缩放比例（使用较长边）
            scale_factor = max(original_width, original_height) / AUTOMORPH_SIZE
...
def run_dr_detection(abs_image_path: str, output_dir: str) -> Dict:
    """
    步骤 3: 硬性渗出物检测（独立任务）- 使用新的 dr_single_image_analyzer

    使用 Attention-UNet 模型进行病灶分割，替代旧的 DR-Detection-and-Clasification-System

    Returns:
        包含硬性渗出物 mask 路径的结果字典
    """
    print("[任务 3/3] 开始: 检测硬性渗出物（使用 dr_single_image_analyzer）...")

    # 获取全局设备配置
    device = get_global_device()
    print(f"  使用设备: {device}")

    # 获取 dr_seg_unet 环境的 Python 路径
    python_path = get_conda_env_python_path('dr_seg_unet')

    # 构建新模型的工作目录
    dr_analyzer_dir = os.path.join(PROJECT_ROOT,"Lesion_detection", "dr_single_image_analyzer")
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"DR 分析失败: {result.stderr}")
    ...
    if start == -1 or end == 0:
        raise RuntimeError(f"脚本输出中未找到 JSON 结构。输出: {stdout}")

    json_str = stdout[start:end]

    try:
        dr_result = json.loads(json_str)
    except json.JSONDecodeError as e:
        raise RuntimeError(f"无法解析 DR 分析的 JSON 输出。错误: {e}")

    # 提取硬性渗出物 mask 路径
    # 新模型输出格式：result['结果目录'] 包含时间戳子目录
    result_dir = dr_result.get("结果目录")
    if not result_dir or not os.path.exists(result_dir):
        raise RuntimeError("DR 分析结果目录未找到")

    hard_exudate_mask_path = os.path.join(result_dir, "hard_exudates_analysis.png")

    if not os.path.exists(hard_exudate_mask_path):
        raise RuntimeError(f"硬性渗出物分割结果未找到: {hard_exudate_mask_path}")

    print("[任务 3/3] 完成: 硬性渗出物检测")

    # 可选：打印额外的诊断信息
    quantitative = dr_result.get('量化分析指标', {})
    if '硬渗出物区域占比' in quantitative:
        print(f"  硬渗出物区域占比: {quantitative['硬渗出物区域占比']:.4f}")
...
def main(image_path: str, output_dir: str = None) -> Dict:
    ...
    print(f"运行设备: {get_global_device()}")  # 🔧 显示全局设备配置
    print()
