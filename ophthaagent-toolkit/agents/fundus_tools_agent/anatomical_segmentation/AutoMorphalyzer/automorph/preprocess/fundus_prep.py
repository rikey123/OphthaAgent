...
def _get_radius_by_mask_center(mask,center):
    ...
    where_result = np.where(b_count > b_count.max() * 0.995)[0]
...
def enhance_dark_image(img, brightness_threshold=60):
    """
    对过暗的图像进行增亮处理

    Args:
        img: 输入图像（灰度或RGB）
        brightness_threshold: 亮度阈值，低于此值则进行增亮
...
def get_mask(img):
    ...
    if g_img.mean() < 60:
        if img.ndim == 3:
            img = enhance_dark_image(img, brightness_threshold=60)
            g_img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        else:
            img = enhance_dark_image(g_img, brightness_threshold=60)
            g_img = img
