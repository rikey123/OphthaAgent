...
def find_latest_lesion_masks(output_dir: str) -> Dict[str, str]:
    """
    在指定的输出目录中查找最新的病灶分割结果。
    ...
    # The output_dir is like '.../results/dr_anchor/intermediate_files'. We need to go up two levels to '.../results'
    # and then join with 'dr_analysis_unet'.
    results_dir = os.path.dirname(os.path.dirname(output_dir))
    dr_analysis_dir = os.path.join(results_dir, "dr_analysis_unet")
    if not os.path.isdir(dr_analysis_dir):
        raise FileNotFoundError(f"Lesion analysis directory not found: {dr_analysis_dir}")

    # 查找所有 'analysis_*' 文件夹
    try:
        analysis_dirs = [d for d in os.listdir(dr_analysis_dir) if d.startswith("analysis_") and os.path.isdir(os.path.join(dr_analysis_dir, d))]
    except FileNotFoundError:
...
def draw_etdrs_grid(image: np.ndarray, grid_params: Dict, segmentation_data: Dict, lesion_mask_paths: Dict[str, str]) -> np.ndarray:
    ...
    if disc_mask_path and os.path.exists(disc_mask_path):
        ...
        if disc_mask is not None:
            ...
            overlay[disc_mask > 128] = [65,105,225]  # 浅蓝色视盘
...
def DR_segmentation_anchor(image_path: str, output_path: str) -> Dict:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            future_lesions_unet = executor.submit(find_latest_lesion_masks, intermediate_files_dir)
